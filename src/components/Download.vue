<template>
  <div id = "uploadContainer">
    <el-popover
      ...
      <Vbotton slot="reference" :isWorking = "isUploadEnd" :clickMethod = "routeToDownload" :nameForButton = "'下载'" :isIcon ="false" :vStyle = "'rounded'"></Vbotton>
    </el-popover>
  </div>
...
export default {
  ...
  methods:{
    ...
    routeToDownload(){
      if("/mainpage/download" !== this.$route.fullPath){
        this.$store.commit("updatePath","/mainpage/download");
        //this.$store.commit("updateLastPath",this.$store.state.currentPath);
        this.$router.push("/mainpage/download");
      }
    },
