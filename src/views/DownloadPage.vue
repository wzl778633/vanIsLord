<template>
  <div id="uploadTable">
    ...
    <el-table
        ...
        empty-text = "你没有下载过文件 或 10天内未下载过文件"
    >
      <el-table-column
          label="文件名"
          prop= "name"
...
export default {
  ...
  methods:{
    ...
    deleteRow(name,id){
      this.$confirm('你确定要删除这项下载记录吗？', '你tm到底删不删？', {
  ...
  computed: {
    monitor () {
      return this.$store.state.downloadState.downloadingFiles;
    },
  ...
  watch: {
    monitor:{
      handler: function (){
        this.tableData = this.$store.state.downloadState.downloadingFiles.concat(this.$store.state.downloadState.downloadedFiles);
      },
      //deep:true,
      immediate:true,
    },
